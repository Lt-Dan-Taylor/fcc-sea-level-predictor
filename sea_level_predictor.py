...
def draw_plot():
    ...
    plt.scatter(df['Year'], df['CSIRO Adjusted Sea Level'], color='royalblue', alpha=0.6, s=70)

    # Create first line of best fit
    lin_reg = linregress(x=df['Year'], y=df['CSIRO Adjusted Sea Level'])
    to_2050 = range(df['Year'].min(), 2051)
    
    plt.plot(to_2050, lin_reg.slope * to_2050 + lin_reg.intercept, color='darkorange', linewidth=4)
    ...
    plt.plot(_2000_to_2050, lin_reg_2000.slope * _2000_to_2050 + lin_reg_2000.intercept, color='crimson', linewidth=4, linestyle='dashed')
